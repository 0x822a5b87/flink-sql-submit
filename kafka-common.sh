...
function create_kafka_topic {
    $KAFKA_DIR/bin/kafka-topics.sh --create --zookeeper test.dc.data.woa.com:2181 --replication-factor $1 --partitions $2 --topic $3
}

function drop_kafka_topic {
    $KAFKA_DIR/bin/kafka-topics.sh --delete --zookeeper test.dc.data.woa.com:2181 --topic $1
}

function send_messages_to_kafka {
    echo -e $1 | $KAFKA_DIR/bin/kafka-console-producer.sh --broker-list test.dc.data.woa.com:9092 --topic $2
}
...
function start_kafka_cluster {
  ...
  while [[ $($KAFKA_DIR/bin/zookeeper-shell.sh test.dc.data.woa.com:2181 get /brokers/ids/0 2>&1) =~ .*Node\ does\ not\ exist.* ]]; do
